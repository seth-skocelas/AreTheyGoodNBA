...
class WebService {
    ...
    func getPlayerSeasonStatsURL(playerID: Int, measureType: MeasureType) -> String {
        ...
        if measureType == MeasureType.RegularAdvanced {
            ...
            return "\(BASE_URL)\(PLAYER_SEASON_STATS)\(CURRENT_SEASON)\(SEASON_TYPE_POST)\(LEAGUE_ID)\(MEASURE_TYPE_ADVANCED)\(PER_MODE_GAME)\(PLUS_MINUS_NO)\(PACE_ADJUST_NO)\(RANK)\(OUTCOME)\(LOCATION)\(MONTH)\(SEASON_SEGMENT)\(DATE_FROM)\(DATE_TO)\(OPPONENT_TEAM_ID)\(VS_CONFERENCE)\(VS_DIVISION)\(GAME_SEGMENT)\(PERIOD)\(LAST_N_GAMES)\(PLAYER_ID)\(playerID)"
